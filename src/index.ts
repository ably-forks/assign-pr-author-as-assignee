...
;(async () => {
  try {
    ...
    if (hasAssigneeOrAssignees(context)) {
      return core.info(
        `Assignee/s already exist/s: [${getAssigneeOrAssignees(context)}]`
      )
    }
...
function getAssigneeOrAssignees(context: Context): string | null {
  let assignees: object[] = context?.payload?.pull_request?.assignee
    ? [context?.payload?.pull_request?.assignee]
    : []

  context?.payload?.pull_request?.assignees?.forEach((assignee: object) => {
    assignees.push(assignee)
  })

  if (assignees?.length > 0) {
    return assignees
      .map((assignee: {login: string}) => assignee.login)
      .join(', ')
  }
